...
func TestEncodeInt16Into(t *testing.T) {
	tests := []struct {
		name          string
		val           int16
		initialTarget []byte
	...
	}{
		{
			...
			initialTarget: nil,
			expectReuse:   false,
			expected:      []byte{byte(TypeInt16), 0x00, 0x2a},
		},
		{
			...
			expectReuse:   false,
			expected:      []byte{byte(TypeInt16), 0xff, 0xff},
		},
		{
			...
			expectReuse:   true,
			expected:      []byte{byte(TypeInt16), 0x01, 0x00},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out := EncodeInt16Into(tt.val, tt.initialTarget)
...
func TestEncodeInt16(t *testing.T) {
	...
	expected := []byte{byte(TypeInt16), 0x80, 0x00}
