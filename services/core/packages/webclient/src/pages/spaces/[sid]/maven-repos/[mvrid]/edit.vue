<script lang="ts" setup>
  ...
  import { getMavenRepository } from '@/api/maven/maven'
  ...
  const repository = ref<SpaceMavenRepository>()
  ...
  onMounted(async () => {
    try {
      ...
      repository.value = await getMavenRepository(space.value.id, (route.params as any).mvrid as string)
      useHead({ title: `Edit ${repository.value.name} - ${space.value.title} - FancySpaces` })
    } catch {
      await router.push(`/spaces/${space.value?.slug || (route.params as any).sid}/maven-repos`)
    } finally {
      loading.value = false
    }
  })
