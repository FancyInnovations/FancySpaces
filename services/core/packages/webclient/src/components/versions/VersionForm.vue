<script lang="ts" setup>
  ...
  const platforms = [
    { title: 'Bukkit', value: 'bukkit' }, { title: 'Spigot', value: 'spigot' },
  ...
  async function save () {
    ...
    try {
      ...
      error.value = error_ instanceof Error ? error_.message : 'Unable to save version.'
    ...
    confirmation.confirmation = {
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          notifications.error(error_ instanceof Error ? error_.message : 'Unable to delete file.')
        }
      },
    }
  }
    ...
    <v-col md="8">
      <Card>
        ...
        <v-card-text>
          ...
          <v-combobox
            v-model="supportedPlatforms"
            chips
            class="mb-4"
            closable-chips
            color="primary"
            :disabled="saving"
            hint="Press Enter after each platform version"
            label="Supported platform versions"
            multiple
            persistent-hint
          />
          ...
          <v-file-input
            ...
            color="primary"
            :disabled="saving"
            label="Add or replace release files"
            multiple
            show-size
          />

          <v-list v-if="files.length > 0" class="bg-transparent mt-4">
            <v-list-subheader>Existing files</v-list-subheader>

            <v-list-item v-for="file in files" :key="file.name" :subtitle="formatSize(file.size)" :title="file.name">
              <template #append>
          ...
          <div class="d-flex ga-3 mt-6">
            ...
            <v-btn color="primary" :loading="saving" @click="save">{{
              props.version ? 'Save Changes' : 'Create Version'
            }}
            </v-btn>
          </div>
        </v-card-text>
